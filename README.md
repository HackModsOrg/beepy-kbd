...
> [!IMPORTANT]
> Blepis v2 uses Pin 5 instead of 4 for the IRQ of the RP2040.
> You need to use the correct pin in `config.txt` or the keyboard will not work!
...
- `lcd_backlight` Set lcd brightness from 0 to 255. Write-only. The default brightness is low, but still on. Setting the brightness to maximum will noticeably increase power draw.
...
```bash
  ...
  # for BLEPIS v2
  sudo make install BOARD=BLEPIS_V2

  # otherwise
  sudo make install
```

To remove, run `sudo make uninstall` (with `BOARD` set as explained above), then verify that `/etc/modules` and `/boot/firmware/config.txt` have removed the driver lines.
